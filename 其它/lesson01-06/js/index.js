...
const ItemEditor = React.createClass({
    ...
    changeHandle(event){
       /*
         ...
         */
        /*this.setState({
            // value:event.target.value  这是一种写法
        });*/
    },
...
const List = React.createClass({
    ...
    render(){
        ...
        for(let entity of this.state.list){
            listDOM.push(<Item id={entity[0]} key={entity[0]} onRemove={this.removeItem} vlaue={entity[1].value} />);
        }

        for(let entity of this.state.editList){
            editListDOM.push(<ItemEditor key={entity[0]} onRemove={this.removeItemEditor} id={entity[0]} vlaue={entity[1].value}/>);
        }
